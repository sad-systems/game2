...
__theClass.prototype.clearTile = function(tile) {
    stateStorage.save(['removedTiles', tile.layer.name, tile.x, tile.y]);
...
__theClass.prototype.addTile = function(tile, x, y) {
    stateStorage.save(['addedTiles', tile.layer.name, x, y]);
    //return this.map.putTile(tile, x, y, tile.layer.name);
    return this.map.putTileWorldXY(tile, x, y, tile.width, tile.height, tile.layer.name);
    //tile.alpha = 0;
    //tile.index = -1;
    //tile.layer.dirty = true;
    //tile.destroy();
...
__theClass.prototype.createMap = function (mapName) {
    ...
    for (var i=0; i<map.layers.length; i++) {
        ...
        var worldLayer = map.createLayer(i),
    ...
    //--- Remove inactive saved tiles:
    var removedTiles = stateStorage.getSection('removedTiles');
    for (var id in removedTiles) {
        var tile = removedTiles[id];
        this.map.removeTile(tile[1], tile[2], tile[0]);
    };
