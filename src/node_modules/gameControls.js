...
module.exports = {
    ...
    create: function(game, playerInstance, mapManager, menuScene) {
        ...
        var mp = mobilepadButtons.create({game:game,
            invisibleAlpha: 0.25,
            state:   globals.get('gameControl') ? 1 : 0,
            inverse: globals.get('gameControl') == 2 ? true : false,
            buttons:{
            a:      {
        ...
        var ab = arsenalButtons.create({
            ...
            onDown: null,
            onUp  : function (o) { 
                //--- Put the good:  //console.log('Arsenal: '+o.name, o);
                if (o.owner.tile) {
        ...
        playerInstance.events.on('firechangestate', function(state) { 
            var button = ab.buttons['Fire'].own,
                farmaeName; 
            if (state.ready) {
                farmaeName = 'FireReady.png';
            } else {
                farmaeName = 'Fire.png';
            }
            try {
                button.setFrames(farmaeName, farmaeName, farmaeName, farmaeName);
            } catch (e) {
                console.log('BUG:[gameControl:button.setFrames]', e);
            };
            //--- Add text:
            if (!button._text) {
                var text = ui.createText('', { x:-button.width/2, y:button.height, anchorY:'bottom', style:'textScoreArsenal' } );
                button.addChild(text);
                button._text = text.children[0];
            };
            button._text.setText(state.state);
            // console.log(button._text);
        ...
        //--- Init the Fire state:
        playerInstance.updateFireState();

        //---------------------------------------------------------------------- 
        // Tools buttons:
        //---------------------------------------------------------------------- 
        var audio   = game.extentions.audio;
        var buttons = {
            ...
            Menu:      { onDown: function(o) {
                                //--- Save the game:
                                gameStorage.save(globals.get('gameName'), playerInstance);
                                //--- Goto main menu:
                                game.extentions.sceneManager.next(menuScene); 
                            }
                       },
            Settings:  {}
        };
