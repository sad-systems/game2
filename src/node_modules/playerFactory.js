...
__theClass.prototype.getSomeGoods = function () {
    ...
    if (tile && !this.isThereSomeCover(tile.worldX, tile.worldY, tile.layer.name)) {
        ...
        this.showAnimation('open', function() {
            //--- Save goods:
            //this.collectedGoods.push( Phaser.Utils.extend(true, {}, tile) ); 
            this.collectedGoods.push( this.mapManager.clearTile(tile) ); 
            console.log('GET the GOODS:', tile, this.collectedGoods);
            //--- Clear tile:
            //this.mapManager.clearTile(tile);
            //tile.index = -1;
...
__theClass.prototype.putSomeGoods = function(goodsIndex) {
    ...
    //-------------------------
    //var goodsIndex = goodsTile
    if (goodsIndex && goodsIndex < this.collectedGoods.length) {
        tile = this.collectedGoods.splice(goodsIndex, 1);
    } else {
        tile = this.collectedGoods.pop();
    }
    console.log('PUT the GOODS:', tile, this.collectedGoods); 
    console.log(this.mapManager.addTile(tile, this.player.x, this.player.bottom-1) );
    //-------------------------
    return true;
};
...
__theClass.prototype.doPut = function(goodsIndex) {
    //--- Put:
    this.showAnimation('turn', function() { 
        if (this.putSomeGoods(goodsIndex)) {
            //--- OK:
            this.showAnimation('turnback', this.stopMove.bind(this));
        } else {
            //--- Fail:
