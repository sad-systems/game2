...
__theClass.prototype.createButtons = function () {
    ...
    //--- Auto hide tools:
    if (this.autoHide) this.toolsPressed(this._buttons.Settings.own);
...
__theClass.prototype.toolsPressed = function (btSettings) {
    ...
    if (!btSettings._data) {
        ...
        tweenHide.to({ x: this.x - btSettings.x }, delayHide, easingHide);
        tweenShow.onComplete.add(changeState, this);    
        tweenShow.to({ x: this.x }, delayShow, easingShow);
        btSettings._data = {
            state:  0,
            tweenHide: tweenHide,
            tweenShow: tweenShow
