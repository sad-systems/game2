...
__theClass.prototype.redrawGroup = function () {
    //----------
    this._x       = 0;
    this._y       = 0;
    this.leftSide = this.config.leftSide;
    ...
    //this.leftSide ? this.groupToolsButtons.anchor.set(0,0) : this.groupToolsButtons.anchor.set(1,0);
    //--- Camera fixed:
...
__theClass.prototype.createButtons = function () {
    ...
    this._buttons[this.CONTROL_KEY] = { onDown:function(o){ this.toolsPressed(o); }.bind(this) };
    ...
    //--- Auto hide tools:
    if (this.autoHide) this.toolsPressed(this._buttons[this.CONTROL_KEY].own);
...
__theClass.prototype.addButton = function (button, buttonGroup) {
    ...
    var bt = this.game.make.button(this._x + button.x * this.dx, this._y + button.y, this.config.atlasId, null, this, frameOver, frameNormal, frameDown);
        this._x += bt.width * this.dx;
        this._w  = this._x;
        this._x += this.config.xIndent * this.dx;
...
__theClass.prototype.toolsPressed = function (btSettings) {
    ...
    var oHide = { x: this.x - btSettings.x };
    var oShow = { x: this.x };
    ...
    if (!btSettings._data) {
        ...
        tweenHide.to(oHide, delayHide, easingHide);
        tweenShow.onComplete.add(changeState, this);    
        tweenShow.to(oShow, delayShow, easingShow);
        btSettings._data = {
            state:  0,
            tweenHide: tweenHide,
            tweenShow: tweenShow
    ...
    if (btSettings._data.state) {
        //this.game.tweens.remove(tweenShow);
        //btSettings._data.tweenShow.to({ x: this.x }, delayShow, easingShow);
        //btSettings._data.tweenShow.updateTweenData('x', this.x); 
        btSettings._data.tweenShow.start();
    } else {
        //btSettings._data.tweenHide.to({ x: this.x - btSettings.x }, delayHide, easingHide);
        //btSettings._data.tweenHide.updateTweenData('x', this.x - btSettings.x); 
...
module.exports = {
    ...
    preload: function (game) {
        game.load.atlas('toolButtons', 'assets/sprites/controls/tools1.png', 'assets/sprites/controls/tools.json');
    }
