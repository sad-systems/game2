...
var mobilepadButtons = function (options) {
    ...
    this.buttons = {
        ...
        joystick: {
            //image: { name: 'mobilepadJoystick', mode:1 } // mode =0 -auto size, =1 -original size
            //trackball: true,
            //sectors: 4,
    ...
    this.defaultImages = {
        '':       { name: 'mobilepadButtons', frame:'default.png',  mode:0 },
        joystick: { name: 'mobilepadButtons', frame:'joystick.png', mode:1 } // mode =0 -auto size, =1 -original size
    };
...
//------------------------------------------------------------------------------
    ...
    _hide: function (obj) {
        this.state ? obj.visible = true : obj.visible = false;
        obj.alpha   = this.invisibleAlpha;
    },

    _addImage: function (buttonSet, graphics) { 
        var imageset = buttonSet.image || this.defaultImages[buttonSet.name] || this.defaultImages[''];
        //console.log(imageset, buttonSet.name);
        if (imageset) {
            var image = new Phaser.Sprite(this.game, 0, 0, imageset.name, imageset.frame);
                image.anchor.set(0.5);
            switch (imageset.mode) {
                case 1: // original
                    var d = image.getBounds().height; 
                    buttonSet.cx += ( (buttonSet.d - d) / 2 ) * (this.inverse ? 1 : -1);
                    buttonSet.d = d;
                    buttonSet.r = buttonSet.d / 2;
                    break;
                default: // auto
                    image.scale.set(buttonSet.d / image.getBounds().height);
            }    
    ...
    _drawJoystick: function () {
        ...
        _joystick = this.game.make.graphics(joystick.cx, joystick.cy);
                    this._addImage(joystick, _joystick);
        _joystick.x = joystick.cx;
        _joystick.y = joystick.cy;
    ...
    _findSector: function (angle, joystick) {
        ...
        for (var i in joystick._sectors) {
            ...
            if (angle >= sector.right && angle <= sector.left) {
                return {id:i, right:sector.right + offset, left:sector.left + offset, base:sector.base }; 
            }
        }
        return null;
    },
    
    _joystickSectorPress: function (obj, sector, pointer) {
        
        if (obj._jpressed != sector.id) {
            //--- Release:
            this._joystickSectorRelease(obj);
            //--- Set marker:
            obj._jpressed = sector.id;
            //--- Trigger an event:
            if (obj._params.onDown && typeof(obj._params.onDown[sector.id]) == 'function') 
                obj._params.onDown[sector.id](obj, sector, pointer);
        }
        
    },
    
    _joystickSectorRelease: function (obj) {
        
        if (typeof(obj._jpressed) === undefined) return false; //<--- Nothing to release
        //--- Trigger an event:
        if (obj._params.onUp && typeof(obj._params.onUp[obj._jpressed]) === 'function') 
            obj._params.onUp[obj._jpressed](obj);
        //--- Delete marker:
        delete obj._jpressed;
        
    },
    
    
    _drawJoystickTrackball: function(_jpointer, x, y, diameter, parentDiameter) {
        
        if (parentDiameter) {
            var maxRadius = parentDiameter/2 - diameter/2,
                curRadius = this._getDinstance(0, 0, x, y);
            if (curRadius > maxRadius) {
                var delta = maxRadius / curRadius;
                x *= delta;
                y *= delta;
            }
        }
        ...
        _jpointer.drawCircle(x, y, diameter);
        _jpointer.endFill();
        
    },
    
    _drawJoystickSector: function(_jpointer, radius, startAngle, endAngle) {
        
        //_jpointer.lineStyle(1, 0xffffff, 0.5); 
        _jpointer.beginFill(0xffffff, 0.25);
        _jpointer.arc(0, 0, radius, -startAngle, -endAngle, true);
        _jpointer.endFill();         
    ...
    _drawJoystickPointer: function (pointer, obj) {
        ...
        var b = obj.getBounds(),
            trackballDiameter = obj._params.d / 2;
        
        var _jpointer = obj._jpointer;
        if (_jpointer) _jpointer.destroy();
        
        _jpointer = this.game.make.graphics(b.centerX, b.centerY);
        //_jpointer._parent = obj;
        ...
        if (obj._params._sectors) {
            ...
            if (sector) {
                ...
                if (obj._params.trackball) {
                    this._drawJoystickTrackball(_jpointer, obj._params.r * Math.cos(this._degToRad(sector.base)), -obj._params.r * Math.sin(this._degToRad(sector.base)), trackballDiameter, obj._params.d);
                } else {
                    this._drawJoystickSector(_jpointer, obj._params.r, this._degToRad(sector.right), this._degToRad(sector.left));
                }
                //--- Trigger an event:
                this._joystickSectorPress(obj, sector, pointer);
            }
        } else {
            //--- Free pointer:
            if (obj._params.trackball) {
                //--- Draw trackball:
                this._drawJoystickTrackball(_jpointer, pointer.x - b.centerX, pointer.y - b.centerY, trackballDiameter, obj._params.d);
            } else {
                //--- Draw sector:
                var u = this._degToRad(obj._params.pointerAngle || 45);
                this._drawJoystickSector(_jpointer, obj._params.r, a - u, a + u);
            }
        }
        //-----
    ...
    _setPad: function () {
        ...
        for(var name in this.buttons) {
            this.buttons[name].name = name;  //<--- Add name to the button
        ...
        switch (length) {
            case 1:
                pads[0] = this._extend(pads[0], {cx:x0, cy:y0, d:diameter, r:radius});
                break;
            case 2:
                var offsetY = center / 2.3;
                pads[0] = this._extend(pads[0], {cx:x0 + 0, cy:y0 - offsetY, d:diameter, r:radius});
                pads[1] = this._extend(pads[1], {cx:x0 + 0, cy:y0 + offsetY, d:diameter, r:radius});
                break;
            case 3:
                var offsetY = center / 2.3,
                    offsetX = diameter * 0.9;
                pads[0] = this._extend(pads[0], {cx:x0 + 0, cy:y0 - offsetY, d:diameter, r:radius});
                pads[1] = this._extend(pads[1], {cx:x0 + 0, cy:y0 + offsetY, d:diameter, r:radius});
                pads[2] = this._extend(pads[2], {cx:x0 - offsetX * (this.inverse ? -1: 1), cy: y0 + 0, d:diameter, r:radius});
                break;
            case 4:
                x0      *= 2.5; 
                offset   = center / 1.7;
    ...
    _onMove: function(pointer) {
        ...
        if (pointer.isMouse) {
            ...
            //console.log(pointer, pointer.isMouse);
        }
...
//------------------------------------------------------------------------------
    ...
    setState: function(state) {
        ...
        if (state) {
            ...
            this.game.input.onUp.remove(this._onReleasePointer, this); //--- release
            this.game.input.deleteMoveCallback(this._onMove, this);    //--- press           
        }
...
module.exports = {
    ...
    preload: function (game) {
        game.load.atlas('mobilepadButtons',  'assets/sprites/controls/mpad1.png', 'assets/sprites/controls/mpad1.json');
